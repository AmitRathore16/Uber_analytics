...
Power BI is connected directly with MySQL to create interactive dashboards.
...
Groq LLM converts it to SQL automatically

sql   SELECT SUM(booking_value) FROM bookings WHERE vehicle_type = 'Auto' AND booking_status = 'Completed';

Query is executed against the MySQL database in real time
Result is returned to the user along with the executed query for full transparency
